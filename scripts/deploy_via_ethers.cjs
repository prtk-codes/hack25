...
async function main() {
  ...
  // Load artifacts
  const microJson = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'artifacts', 'contracts', 'tokens', 'MicroUSD.sol', 'MicroUSD.json')));
  const hubJson = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'artifacts', 'contracts', 'core', 'PaymentHub.sol', 'PaymentHub.json')));
